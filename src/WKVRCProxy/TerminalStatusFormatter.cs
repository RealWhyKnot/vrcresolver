...
internal static class TerminalStatusFormatter
{
        ...
        bool unicodeSymbols = true)
    {
        ...
        bool whyKnotAnimating = snapshot.WhyKnotActive(nowUtc, TerminalRefreshPolicy.AnimationWindow);
        bool relayRecent = snapshot.RelayActive(nowUtc, TerminalRefreshPolicy.RecentActivityWindow);
        bool whyKnotRecent = snapshot.WhyKnotActive(nowUtc, TerminalRefreshPolicy.RecentActivityWindow);
        ...
        bool includeSparkline)
    {
        ...
        runs.Add(new TerminalTextRun("  WhyKnot", ConsoleColor.Gray));
        string whyKnotGlyph = TerminalEffectEngine.ActivityGlyph(glyphs, whyKnotAnimating, spinnerIndex + 1, animationsEnabled);
        if (whyKnotGlyph.Length > 0)
        {
            runs.Add(new TerminalTextRun(" ", ConsoleColor.DarkGray));
            runs.Add(new TerminalTextRun(whyKnotGlyph, TerminalEffectEngine.PulseColor(spinnerIndex, 2, animationsEnabled)));
        }
        runs.Add(new TerminalTextRun(" ", ConsoleColor.DarkGray));
        if (whyKnotRecent)
        {
            runs.Add(new TerminalTextRun("pulling", TerminalEffectEngine.PulseColor(spinnerIndex, 3, whyKnotAnimating && animationsEnabled)));
            if (includeTotals && snapshot.WhyKnotRelayBytesTotal > 0)
            {
                runs.Add(new TerminalTextRun(" ", ConsoleColor.DarkGray));
                runs.Add(new TerminalTextRun(WatchdogDisplay.FormatBytes(snapshot.WhyKnotRelayBytesTotal) + " received", ConsoleColor.Gray));
            }
        }
        else
        {
