...
internal static class LocalRelayTlsManager
{
    ...
    private const string FriendlyName = "WKVRCProxy localhost.youtube.com relay";
    ...
    private static IEnumerable<int> ReadRecordedPorts()
    {
        ...
        AddPortFromFile(Path.Combine(WkvrcPaths.StateRoot(), "relay_last_port.txt"), ports);
        AddPortFromFile(Path.Combine(WkvrcPaths.StateRoot(), "relay_port.txt"), ports);
        string path = PortsPath();
        if (File.Exists(path))
        {
    ...
    {
        string? value = Environment.GetEnvironmentVariable("WKVRCPROXY_DISABLE_LOCAL_RELAY_HTTPS");
    ...
    private static string ProgramDataRoot()
        => Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "WKVRCProxy");
