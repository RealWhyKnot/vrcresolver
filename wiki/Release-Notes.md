...
- **feat(relay): localhost.youtube.com HTTP trust gateway.** Watchdog binds an ephemeral high port on 127.0.0.1; the wrapper rewrites resolved URLs to `http://localhost.youtube.com:{port}/play/<session>/manifest.<ext>?target=<base64>`. AVPro's trust list accepts the URL because the hosts file pins `localhost.youtube.com -> 127.0.0.1`. The relay forwards bytes and relative playlist subrequests without parsing manifests; WhyKnot.dev owns HLS compatibility and transcode decisions. AOT-clean. HTTPS + cert lifecycle is parked behind concrete trigger conditions.
