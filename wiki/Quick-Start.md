...
- `[relay] listening port {port}` -- the trust-gateway HTTP listener bound to 127.0.0.1 and wrote the port to `relay_port.txt`. The wrapper reads this on every resolve and routes the URL through `localhost.youtube.com:{port}` so AVPro's allowlist accepts it. See [[Trust Gateway]] for the full flow.
